...
You can find more ideas around this on [#955](https://github.com/otghcloud/nspanel-ha/issues/955).
...
#### Use Case
One application, as utilized by @tikismoke and detailed in [#1349](https://github.com/otghcloud/nspanel-ha/issues/1349), is in response to fluctuating energy prices.
...
switch:
  - name: Relay 1 Local
    ...
    internal: false
    lambda: |-
      return (id(relay_settings) & nspanel_ha::RelaySettings::Relay1_Local);
    turn_on_action:
      - lambda: nspanel_ha::update_bitwise_setting(id(relay_settings), true, RelaySettings::Relay1_Local);
    on_turn_on:
      - logger.log: "Relay 1 Local turned On!"
    turn_off_action:
      - lambda: nspanel_ha::update_bitwise_setting(id(relay_settings), false, RelaySettings::Relay1_Local);
    on_turn_off:
  ...
  - name: Relay 2 Local
    ...
    entity_category: config
    internal: false
    lambda: return (id(relay_settings) & nspanel_ha::RelaySettings::Relay2_Local);
    turn_on_action:
      - lambda: nspanel_ha::update_bitwise_setting(id(relay_settings), true, RelaySettings::Relay2_Local);
    on_turn_on:
      - logger.log: "Relay 2 Local turned On!"
    turn_off_action:
      - lambda: nspanel_ha::update_bitwise_setting(id(relay_settings), false, RelaySettings::Relay2_Local);
    on_turn_off:
...
like to control a cover like discussed in [#965](https://github.com/otghcloud/nspanel-ha/issues/965).
