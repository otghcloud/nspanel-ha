...
#ifdef NSPANEL_HA_PAGE_UTILITIES
...
#elif defined(USE_ARDUINO)
  UtilitiesGroups =
      static_cast<UtilitiesGroupValues *>(ps_malloc(UTILITIES_GROUPS_COUNT * sizeof(UtilitiesGroupValues)));
#endif  // USE_ESP_IDF

  if (!UtilitiesGroups)
    UtilitiesGroups = new UtilitiesGroupValues[UTILITIES_GROUPS_COUNT];
  if (!UtilitiesGroups)
    return;

  static constexpr UtilitiesGroupValues INITIAL_UTILITIES_GROUPS[UTILITIES_GROUPS_COUNT] = {
      {"grid", "", "", 0},  // Use "" instead of "\0" for clarity
      {"group01", "", "", 0}, {"group02", "", "", 0}, {"group03", "", "", 0}, {"group04", "", "", 0},
      {"group05", "", "", 0}, {"group06", "", "", 0}, {"home", "", "", 0}};
...
#endif  // NSPANEL_HA_PAGE_UTILITIES
